...
static int is_io_addr(uint16_t addr) {
    return (addr >= IO_START && addr <= IO_END);
}

static uint8_t if_read8(uint16_t addr) {
    if (is_io_addr(addr)) {
...
static void if_write8 (int16_t addr, uint8_t v) {
    if (is_io_addr(addr)) {
        io_write8(addr, v);
    }
    else {
    ...
    */
    ...
    switch (in.op) {
        ...
        case OP_CMPW:
            check_reg(in.ra);
            check_reg(in.b2);
            cpu->zf = (cpu->r[in.b2] == cpu->r[in.ra]);
            cpu->pc += 4;
            break;
        case OP_JMP:
        ...
        case OP_MRMOVB: 
            check_reg(in.ra);
            cpu->r[in.ra] = (uint16_t)if_read8(u16_from_le(in.b2, in.b3));
            cpu->zf = (cpu->r[in.ra] == 0);
            cpu->pc += 4;
            break;
        case OP_RMMOVB:
            check_reg(in.ra);
            if_write8(u16_from_le(in.b2, in.b3), (uint8_t)(cpu->r[in.ra] & 0xFF));
            cpu->pc += 4;
            break;
        case OP_MRMOVBR:
            check_reg(in.ra);
            check_reg(in.b2);
            cpu->r[in.ra] = (uint16_t)if_read8(cpu->r[in.b2]);
            cpu->zf = (cpu->r[in.ra] == 0);
            cpu->pc += 4;
            break;
        case OP_RMMOVBR: 
            check_reg(in.ra);
            check_reg(in.b2);
            if_write8(cpu->r[in.b2], (uint8_t)(cpu->r[in.ra] & 0xFF));
            cpu->pc += 4;
            break;
        case OP_MRMOVW:
            check_reg(in.ra);
